...
local aspects_config = require("aspects/aspects_config")

-- Aspects are configured in one place: scripts/vscripts/aspects/aspects_config.lua.
-- Keep the legacy list shape so current player saves and shop records continue to work.
game_spells_lib.aspects_config = aspects_config
game_spells_lib.spells_list = aspects_config:GetLegacySpellList()
...
game_spells_lib.spells_cost_random = aspects_config:GetRandomAspectCost()
game_spells_lib.SPELL_MAX_TIME_TO_ACTIVE = aspects_config.SPELL_MAX_TIME_TO_ACTIVE
game_spells_lib.SPELL_MAX_LEVEL = aspects_config:GetMaxLevel()
game_spells_lib.BASE_UPGRADE_STEP_COST = aspects_config:GetBaseUpgradeStepCost()
...
CustomNetTables:SetTableValue("game_spells_lib", "aspects_config", aspects_config:BuildNetTable())
...
function game_spells_lib:event_set_activate_spell(data)
    ...
    if game_spells_lib:FindCurrentSpellPlayer(player_id, spell_name) then
        ...
        if was_solo_before or is_solo_now or player_team == 3 or GameRules:IsCheatMode() then
            max_allowed = game_spells_lib.SPELL_MAX_LEVEL
        end
...
function game_spells_lib:event_buy_spell(data)
    ...
    if find_new_spell then
        ...
        if drop_info ~= nil and (drop_info[2] > 0 and drop_info[2] <= game_spells_lib.SPELL_MAX_LEVEL) then
            CustomGameEventManager:Send_ServerToPlayer( player, 'event_spell_shop_drop', {spell_name = drop_info[1], upgrade = drop_info[2]} )
        end
    end

    if drop_info ~= nil and drop_info[2] <= game_spells_lib.SPELL_MAX_LEVEL and drop_info[2] > 0 then
        ...
        elseif drop_info[2] > 0 and drop_info[2] <= game_spells_lib.SPELL_MAX_LEVEL then
...
function game_spells_lib:PlayerUpgradeSpell(player_id, idPerk)
    ...
    if #find_random_to_upgrade <= 0 then
        for i=1,GetTableLng(game_spells_lib.PLAYER_INFO[player_id])-1 do
            if tonumber(game_spells_lib.PLAYER_INFO[player_id][tostring(i)]["2"]) < game_spells_lib.SPELL_MAX_LEVEL then
                for _, spell_history in pairs(game_spells_lib.spells_list) do
                    if spell_history[1] == game_spells_lib.PLAYER_INFO[player_id][tostring(i)]["1"] and tonumber(spell_history[6]) == idPerk then
                        table.insert(find_random_to_upgrade, game_spells_lib.PLAYER_INFO[player_id][tostring(i)]["1"])
                    end
...
function game_spells_lib:GetSpellCost(player_id, spell_name, level)
    ...
    if target_level > game_spells_lib.SPELL_MAX_LEVEL then return 0 end
        ...
        for i = 1, target_index - 1 do
            ...
            if tostring(def[6]) == target_side then
                ...
                if aspect_level < 2 then
                    cost = cost + game_spells_lib.BASE_UPGRADE_STEP_COST
                end
            end
        end

        cost = cost * (1 - aspects_config:GetUpgradeDiscount(2))
    ...
    elseif target_level == 3 then
        ...
        for i = 1, target_index - 1 do
            ...
            if tostring(def[6]) == target_side then
                ...
                if aspect_level < game_spells_lib.SPELL_MAX_LEVEL then
                    cost = cost + game_spells_lib.BASE_UPGRADE_STEP_COST
                end
            end
        end

        cost = cost * (1 - aspects_config:GetUpgradeDiscount(3))
    end
    if GameRules:IsCheatMode() and not GameRules.isTesting then
       return -2 
    end

    return math.max(game_spells_lib.BASE_UPGRADE_STEP_COST, math.floor(cost + 0.5)) -- округление до целого
end
...
function game_spells_lib:PlayerUpgradeSpellSelected(player_id, spell_name)
    ...
    for i=1,GetTableLng(game_spells_lib.PLAYER_INFO[player_id])-1 do
        ...
        if info["1"] == spell_name then
            ...
            if current >= game_spells_lib.SPELL_MAX_LEVEL then return nil end
            info["2"] = current + 1
...
function game_spells_lib:event_upgrade_spell(data)
    ...
    if level >= game_spells_lib.SPELL_MAX_LEVEL then return end
...
function game_spells_lib:SetSpellPlayers(id)
    ...
    elseif team == DOTA_TEAM_BADGUYS then
        if #active == 0 then
            ...
            local picked = {}
            while #picked < game_spells_lib.SPELL_MAX_LEVEL and #candidates > 0 do

        ...
        if type(info) == "table" then
            ...
            if isBuilding then
                ...
                if info['GoldAmount'] then
                    values.gold_gain = info['GoldAmount'] * GameRules.MapSpeed
                    CustomNetTables:SetTableValue("abilities", name, {
                        gold_amount = info["GoldAmount"] * GameRules.MapSpeed
                    })
                end
                ...
                if info['UnitName'] then
                    ...
                    values.gold_cost = info['AbilityValues']['gold_cost']
                    values.lumber_cost =
                    info['AbilityValues']['lumber_cost']
                    CustomNetTables:SetTableValue("buildings", name, values)
                ...
                local bonus_attr
                if GetItemKV(itemName)["AbilityValues"] then
                    for key, value in pairs(
                        GetItemKV(itemName)["AbilityValues"]) do
                        if string.match(key, "bonus") then
                            bonus_attr = key
                        end
                    end
                    local bonus_values =
                    GetItemKV(itemName)["AbilityValues"][bonus_attr]
                    local gold =
                    GetItemKV(itemName)["AbilityValues"]["gold_cost"]
                    local lumber =
                    GetItemKV(itemName)["AbilityValues"]["lumber_cost"]
                ...
                CustomNetTables:SetTableValue("abilities", name, {
                    ...
                    amount = info["LumberAmount"] * GameRules.MapSpeed
                })
...
function BuildingHelper:OnGameRulesStateChange(keys)
    ...
    if newState == DOTA_GAMERULES_STATE_CUSTOM_GAME_SETUP then
        ...
        -- 1 
        Timers:CreateTimer(TEAM_CHOICE_TIME-9, function()
...
function BuildingHelper:UpdateGrid()
    BuildingHelper.Grid = {} -- Construction grid
    BuildingHelper.Terrain = {} -- Terrain grid, this only changes when a tree is cut
    BuildingHelper.Encoded = "" -- String containing the base terrain, networked to clients
    BuildingHelper.squareX = 0 -- Number of X grid points
    ...
    if GameRules.trollHero and GameRules.trollID then
        ...
        for _, unit in pairs(units) do
            ...
            if string.match(unit_name, "shop") or string.match(unit_name, "troll_hut") then
                unit:SetOwner(GameRules.trollHero)
                unit:SetControllableByPlayer(GameRules.trollID, true)
                unit:AddNewModifier(unit, nil, "modifier_invulnerable", {})
                unit:AddNewModifier(unit, nil, "modifier_phased", {})
                if string.match(unit_name, "troll_hut") then
                    unit.ancestors = {}
                    ModifyStartedConstructionBuildingCount(GameRules.trollHero, unit_name, 1)
                    ModifyCompletedConstructionBuildingCount(GameRules.trollHero, unit_name, 1)
...
function BuildingHelper:InitGNV()
    local worldMin = Vector(-12000, -12000, 0)
    local worldMax = Vector(12000, 12000, 0)
...
function BuildingHelper:RepairCommand(args)
    ...
    for _, entityIndex in pairs(selectedEntities) do
        local unit = EntIndexToHScript(entityIndex)
        if unit ~= nil and playerID == unit:GetPlayerOwnerID() then
...
function BuildingHelper:RightClickOrder(event)
    ...
    if unit:GetPlayerOwnerID() ~= pID then
        return
    end
    -- Repair
...
function BuildingHelper:OrderFilter(order)
    ...
    if order_type == DOTA_UNIT_ORDER_CAST_NO_TARGET then
        if units["0"] then
            ...
            if string.match(unit_name, "shop") or string.match(unit_name, "troll_hut") or string.match(unit_name, "tower_buff_good")  then
                shop.buyer = issuerID
            ...
            if string.match(unit_name, "shop") or string.match(unit_name, "troll2_hut") or string.match(unit_name, "tower_buff_bad")  then
                shop.buyer = issuerID
                if string.match(unit_name, "troll2_hut") and string.match(EntIndexToHScript(abilityIndex):GetAbilityName(),"upgrade_to") and PlayerResource:GetSelectedHeroEntity(issuerID):GetUnitName() ~= TROLL_HERO and GameRules.test2 == false then
                    SendErrorMessage(issuerID, "error_only_troll_can_upgrade")
                    return false
                end
            end
    ...
    if order_type == DOTA_UNIT_ORDER_PURCHASE_ITEM then
        SendErrorMessage(issuerID, "error_no_buy_item")
        return false
    end
...
function BuildingHelper:AddBuilding(keys)
    ...
    if builder:IsElf() then
        PlayerResource:NewSelection(builder:GetPlayerOwnerID(), builder)
    end
...
function BuildingHelper:UpgradeBuilding(building, newName)
    ...
    if building:HasModifier("modifier_buff_glyph_counter") then
        local glyphStackModifier = building:FindModifierByName("modifier_buff_glyph_counter")
        if glyphStackModifier then
            local remainingTime = glyphStackModifier:GetRemainingTime()
            newBuilding:AddNewModifier(newBuilding, nil, "modifier_buff_glyph_counter",{duration = remainingTime})
        end
    end
...
function BuildingHelper:ClearWork(work)
    if work then
            ...
            function()
                ...
                if not IsValidEntity(target) or not target:IsAlive() then
                    ...
                    if target.upgraded and target.units_repairing then
                        ...
                        if not IsValidEntity(repair_ability) and target then
...
function BuildingHelper:IsInsideBaseArea(unit, location, nameBuilding, build)
    ...
    elseif nameBuilding == "flag" then
        if unit.work then 
            BuildingHelper:ClearWork(unit.work)
        end
        SendErrorMessage(playerID, "error_place_is_flag")
        return false
    else 

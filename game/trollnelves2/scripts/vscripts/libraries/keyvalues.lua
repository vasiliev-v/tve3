...
--[[
    ...
    - Ability Special value grabbing:
        ...
        With a handle:
            ability:GetAbilityValues("some_key") -- returns based on the level of the ability/item
        With string:
            GetAbilityValues("some_custom_ability", "some_key")    -- returns "-3 -4 -5" (string)
            GetAbilityValues("some_custom_ability", "some_key", 2) -- returns -4 (number)
    Notes:
...
function CDOTABaseAbility:GetAbilityValues(key)
    return GetAbilityValues(self:GetAbilityName(), key, self:GetLevel())
end
...
function GetAbilityValues(name, key, level)
        ...
        if type(info) == "table" then
            ...
            elseif isBuildingAbility then
                ...
                if info['UnitName'] then
                    ...
                    values.lumber_cost = info['AbilityValues']['lumber_cost']
                    CustomNetTables:SetTableValue("buildings",name, values)
            ...
            elseif info["BuyItem"] then
                ...
                if GetItemKV(itemName)["AbilityValues"] then
                    for key,value in pairs(GetItemKV(itemName)["AbilityValues"]) do
                        if string.match(key,"bonus") then
                            bonus_attr = key
                        end
                    end
                    local bonus_values = GetItemKV(itemName)["AbilityValues"][bonus_attr]
                    local gold = GetItemKV(itemName)["AbilityValues"]["gold_cost"]
                    local lumber = GetItemKV(itemName)["AbilityValues"]["03"]["lumber_cost"]
                    ...
                    local gold = GetItemKV(itemName)["AbilityValues"]["gold_cost"]
                    local lumber = GetItemKV(itemName)["AbilityValues"]["lumber_cost"]
            ...
            elseif info["LumberAmount"] and info["LumberInterval"] then
                CustomNetTables:SetTableValue("abilities",name, { lumber_interval = info["LumberInterval"],amount = info["LumberAmount"] * GameRules.MapSpeed  })
            elseif string.match(name,"train_") then
                CustomNetTables:SetTableValue("abilities",name, { gold_cost = info['AbilityValues']['gold_cost'],lumber_cost = info['AbilityValues']['lumber_cost'],food_cost = info['AbilityValues']['food_cost'] })

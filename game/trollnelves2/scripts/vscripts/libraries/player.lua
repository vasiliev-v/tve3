...
function CDOTA_PlayerResource:SetGold(hero, gold)
    local status, nextCall = Error_debug.ErrorCheck(function() 
        ...
        local limitGold = 0
        local limitLumber = 0  -- добавлено для согласованности, хотя в SetGold не используется

        if hero:HasModifier("modifier_troll_spell_limit_gold") then
            local mod = hero:FindModifierByName("modifier_troll_spell_limit_gold")
            if mod then
                local s = mod:GetStackCount()
                if s == 1 then limitGold = 200000
                elseif s == 2 then limitGold = 275000
                elseif s == 3 then limitGold = 350000 end
            end
        end

        if hero:HasModifier("modifier_troll_spell_limit_gold_x4") then
            local mod = hero:FindModifierByName("modifier_troll_spell_limit_gold_x4")
            if mod then
                local s = mod:GetStackCount()
                if s == 1 then limitGold = 200000
                elseif s == 2 then limitGold = 350000
                elseif s == 3 then limitGold = 550000 end
            end
        end

        if hero:HasModifier("modifier_elf_spell_limit_gold") then
            local mod = hero:FindModifierByName("modifier_elf_spell_limit_gold")
            if mod then
                local s = mod:GetStackCount()
                if s == 1 then limitGold = 150000; limitLumber = 150000
                elseif s == 2 then limitGold = 250000; limitLumber = 250000
                elseif s == 3 then limitGold = 350000; limitLumber = 350000 end
            end
        end

        if hero:HasModifier("modifier_elf_spell_limit_gold_x4") then
            local mod = hero:FindModifierByName("modifier_elf_spell_limit_gold_x4")
            if mod then
                local s = mod:GetStackCount()
                if s == 1 then limitGold = 250000; limitLumber = 250000
                elseif s == 2 then limitGold = 350000; limitLumber = 350000
                elseif s == 3 then limitGold = 500000; limitLumber = 500000 end
            end
        end
...
function CDOTA_PlayerResource:SetLumber(hero, lumber)
    local status, nextCall = Error_debug.ErrorCheck(function() 
        local playerID = hero:GetPlayerOwnerID()
        local limitLumber = 0
        local limitGold = 0  -- добавлено для согласованности, хотя в SetLumber не используется

        if hero:HasModifier("modifier_elf_spell_limit_lumber") then
            local mod = hero:FindModifierByName("modifier_elf_spell_limit_lumber")
            if mod then
                local s = mod:GetStackCount()
                if s == 1 then limitLumber = 200000
                elseif s == 2 then limitLumber = 400000
                elseif s == 3 then limitLumber = 600000 end
            end
        end

        if hero:HasModifier("modifier_elf_spell_limit_lumber_x4") then
            local mod = hero:FindModifierByName("modifier_elf_spell_limit_lumber_x4")
            if mod then
                local s = mod:GetStackCount()
                if s == 1 then limitLumber = 200000
                elseif s == 2 then limitLumber = 350000
                elseif s == 3 then limitLumber = 500000 end
            end
        end

        -- Убрана лишняя локальная переменная внутри блока
        if hero:HasModifier("modifier_elf_spell_limit_gold") then
            local mod = hero:FindModifierByName("modifier_elf_spell_limit_gold")
            if mod then
                local s = mod:GetStackCount()

...
local winter_drop = {"item_event_winter_firework", "item_event_winter_snowball", "item_event_winter_cinder","item_event_winter_snowman", "item_event_winter_santa_hat", 
					"item_vip", 
...
function TimerRandomDropWinter(event)
	...
	Timers:CreateTimer(function()
		if countGift < maxGift then
			local randTime = RandomInt( 30, 61 )
			local spawnPoint = unit:GetAbsOrigin()	
			local itemdrop =  RandomInt( 1, #winter_drop)
			local newItem = CreateItem( winter_drop[itemdrop], nil, nil )
			local dropRadius = RandomFloat( 10, 360 )
			local randRadius = spawnPoint + RandomVector( dropRadius )
			local drop = CreateItemOnPositionForLaunch( randRadius, newItem )
			newItem:LaunchLootInitialHeight( false, 0, 150, 0.5, randRadius )
			newItem:SetContextThink( "KillLoot", function() return KillLoot( newItem, drop ) end, 300 )
			countGift = countGift + 1
			if countGift > 2 then
				MinimapEvent(DOTA_TEAM_BADGUYS, unit, unit:GetAbsOrigin().x, unit:GetAbsOrigin().y, DOTA_MINIMAP_EVENT_HINT_LOCATION, 10 )
				AddFOWViewer(DOTA_TEAM_BADGUYS, unit:GetAbsOrigin(), 1200, 10, false)
			end
			return randTime
		end	
	end)

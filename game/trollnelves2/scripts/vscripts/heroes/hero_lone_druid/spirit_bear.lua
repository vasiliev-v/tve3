...
function SpiritBearSpawn( event )
	...
	for _,unit in pairs(units) do
		...
		if unit_name_hut == "troll_hut_6" or unit_name_hut == "troll2_hut_6" then --if unit_name_hut == "troll_hut_7" then
			checkHut = true
		end
	end
	if checkHut then
		...
		if caster.bear and IsValidEntity(caster.bear) and not caster.bear:IsAlive() then
			...
			for i=6, 8 do
				local item = caster.bear:GetItemInSlot(i)
				if item ~= nil then
		...
		if caster.bear and IsValidEntity(caster.bear) and caster.bear:IsAlive() then
			...
			caster.bear:RemoveAbility("bear_desolator")
			caster.bear:RemoveAbility("bear_return")
		...
		wearables:SetBear(player)	
	else 
		ability:EndCooldown()
		SendErrorMessage(player, "error_need_buy_hut_6") --SendErrorMessage(player, "error_need_buy_hut_7")
	end
end
...
function SpiritBearDeath( event )
	...
	ability:EndCooldown()
	ability:StartCooldown(300/GameRules.MapSpeed) --300

...
var SEASON_TAB_SCROLL_OFFSET = 0
...
function RenderSeasonTabs(preferredSelection)
{
    ...
    let viewport = $("#SeasonTabsViewport")
    if (!container || !wrapper || !viewport)
    {
        return
    }
    ...
    {
        RefreshSeasonTabScroll()
        EnsureSeasonTabVisible(selectionKey)
    })
}
...
function SelectSeason(seasonKey)
{
    ...
    $.Schedule(0.0, function()
    {
        EnsureSeasonTabVisible(seasonKey)
    })
}

function ScrollSeasonTabs(direction)
{
    if (direction === 0)
    {
        return
    ...
    let viewportWidth = GetPanelWidth(viewport)
    if (viewportWidth <= 0)
    {
        return
    }

    let contentWidth = GetPanelWidth(tabs)
    let maxOffset = Math.max(0, contentWidth - viewportWidth)
    if (maxOffset <= 0)
    {
        SEASON_TAB_SCROLL_OFFSET = 0
        RefreshSeasonTabScroll()
    ...
    let step = Math.max(120, viewportWidth * 0.8)
    SEASON_TAB_SCROLL_OFFSET += step * direction
    if (SEASON_TAB_SCROLL_OFFSET < 0)
    {
        SEASON_TAB_SCROLL_OFFSET = 0
    }
    if (SEASON_TAB_SCROLL_OFFSET > maxOffset)
    {
        SEASON_TAB_SCROLL_OFFSET = maxOffset
    }

    RefreshSeasonTabScroll()
}

function RefreshSeasonTabScroll()
{
    let viewport = $("#SeasonTabsViewport")
    let tabs = $("#SeasonTabs")
    let prevButton = $("#SeasonTabsPrev")
    let nextButton = $("#SeasonTabsNext")
    ...
    else
    {
        ...
        tabs.style.marginLeft = (-SEASON_TAB_SCROLL_OFFSET) + "px"
    }

    if (prevButton)
    {
        prevButton.SetHasClass("Hidden", maxOffset <= 0 || SEASON_TAB_SCROLL_OFFSET <= 0)
    }
    if (nextButton)
    {
        nextButton.SetHasClass("Hidden", maxOffset <= 0 || SEASON_TAB_SCROLL_OFFSET >= maxOffset - 1)
    }
}

function EnsureSeasonTabVisible(seasonKey)
{
    let viewport = $("#SeasonTabsViewport")
    let tabs = $("#SeasonTabs")
    let button = SEASON_BUTTONS[seasonKey]
    if (!viewport || !tabs || !button)
    {
        return
    }
    ...
    let viewportPos = viewport.GetPositionWithinWindow ? viewport.GetPositionWithinWindow() : [0, 0]
    let buttonPos = button.GetPositionWithinWindow ? button.GetPositionWithinWindow() : [0, 0]
    let buttonLeft = buttonPos[0] - viewportPos[0]
    let buttonRight = buttonLeft + GetPanelWidth(button)

    if (buttonLeft < 0)
    {
        SEASON_TAB_SCROLL_OFFSET = Math.max(0, SEASON_TAB_SCROLL_OFFSET + buttonLeft - 8)
    }
    else if (buttonRight > viewportWidth)
    {
        SEASON_TAB_SCROLL_OFFSET = Math.min(maxOffset, SEASON_TAB_SCROLL_OFFSET + (buttonRight - viewportWidth) + 8)
    }
...
function GetPanelWidth(panel)
{
    ...
    {
        return panel.actuallayoutwidth
    }

    if (panel.actualuisize !== undefined && panel.actualuisize.length > 0)
    {
        return panel.actualuisize[0]
    }

                ...
                if (data[poriadok[x-1]+1] != null)
                {
                    ...
                    if (data[poriadok[x-1]+1] != "nill")
                    {
                        if (data[poriadok[x-1]+1] != "normal")
                        {
                            ...
                            {
                                $.CreatePanel(`Panel`, $("#NewPartButt"+myint), `NewTooltip${myint}`, {
                                    class: `NewTooltip`,
                    ...
                    else
                    {
                        if (needs[poriadok[x-1]] != false)
                        {
                            ...
                            if ($("#NewTooltip"+myint) == null)
                            {
                                $.CreatePanel(`Panel`, $("#NAPartButt"+myint), `NewTooltip${myint}`, {
                                    class: `NewTooltip`,
                                    onmouseover: `UIShowCustomLayoutParametersTooltip(ParticleTooltip${myint},file://{resources}/layout/custom_game/pets_tooltips.xml,num=${poriadok[x-1]+1}`, 
                                    onmouseout: `UIHideCustomLayoutTooltip(ParticleTooltip${myint})`,
                                });
...
(function()
{
    GameEvents.SubscribeProtected( "DefaultButtonReadyPets", DefaultButtonReady);
    GameEvents.SubscribeProtected( "SetSelectedPets", SetSelectedParticles);
    for (var x = 1; x < names.length+1; x = x + 1)
    {
        var stl = (100*(x-1)) + 20
        $.CreatePanel(`Button`, $("#CustomUIContainer"), `NewPartButt${x}`, {
            class: `NewPartButt`,
            style: `margin-top:${stl}px;`,
            onactivate: `SelectPart(${x})`, 
        });
        $.CreatePanel("Image", $("#NewPartButt"+x), `pereg${x}`, { src:"file://{images}/custom_game/all/st.png", style: "width:5px; margin-top:0px; margin-left:150px;" });
        $.CreatePanel("Image", $("#NewPartButt"+x), `partapngb${x}`, { src: "file://{images}/custom_game/all/activbutt.png"});   
        $.CreatePanel("Label", $("#NewPartButt"+x), `NewPartNum${x}`, {text: '0' });
        $.CreatePanel("Label", $("#NewPartButt"+x), `partname${x}`, { style: "margin-top:20px; margin-left:10px;", text: "Название эффекта" });
        $.CreatePanel("Label", $("#NewPartButt"+x), `partnote${x}`, { style: "margin-top:5px; margin-left:180px;", text: "Описание/причина выдачи/примечание" });
        ...
        $.CreatePanel(`Button`, $("#CustomUIContainer"), `NAPartButt${x}`, {
            class: `NAPartButt`,
            style: `margin-top:${stl}px;`,
            onactivate: `SelectPart(${x})`, 
        });
        $.CreatePanel("Image", $("#NAPartButt"+x), `napereg${x}`, { src: "file://{images}/custom_game/all/st.png", style: 'width:5px; margin-top:0px; margin-left:150px;'});  
        $.CreatePanel("Label", $("#NAPartButt"+x), `napartname${x}`, { style: "margin-top:20px; margin-left:10px;", text: "Название эффекта" });
        $.CreatePanel("Label", $("#NAPartButt"+x), `napartnote${x}`, { style: "margin-top:5px; margin-left:180px;", text: "Описание/причина выдачи/примечание" });
        $("#NAPartButt"+x).visible = false;
    }
